...
Player::Player(const Vector3f& position, float rotX, float rotY) :m_position(position), xrot(rotX), yrot(rotY), diffx(),diffy(),lastx(),lasty()
{
...
Vector3f Player::SimulateMove(bool front, bool back, bool left, bool right, bool space, bool shift, float elapsedTime, bool collisionY)
{
   ...
   {
      if ((m_now - m_last) < 1.5f)
      {
         delta.y += elapsedTime * 40;
         m_now = m_position.y;

      }
      else if ((m_now - m_last) >= 1.5f)
      {
         m_saut = true;
      }
   }
