...
namespace Hevadea.Items.Tags
{
    public abstract class PlacableItemTag : InteractItemTag
    {
        ...
        public override void InteracteOn(Entity user, Coordinates coords)
        {
            ...
            string s = user.HoldedItem().Name;
            if ((e == null || e.HasComponent<ComponentBreakable>())&& ((user.HoldedItem().Name == "Boat" &&( user.Level.GetTile(coords).HasTag<LiquideTile>()|| !user.Level.GetTile(coords).HasTag<LiquideTile>())) || (user.HoldedItem().Name != "Boat" && !user.Level.GetTile(coords).HasTag<LiquideTile>())) &&
